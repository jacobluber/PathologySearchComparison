...
def process_slide(row, data_dir, save_dir):
    patch_save_dir = join(save_dir, splitext(row["file_name"])[0])
    makedirs(patch_save_dir, exist_ok=True)
    
    # for test delete row["id"] and run:
    # python parallel_patching.py --data_dir ./TEST_DATA --metadata_path test_metadata.csv --save_dir ./TEST_PATCHES --num_processes 6
...
if __name__ == "__main__":
    ...
    with Pool(args.num_processes) as pool:
        f = partial(process_slide, data_dir=data_dir, save_dir=save_dir)
