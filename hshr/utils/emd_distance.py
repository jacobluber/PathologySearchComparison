...
# Flow matrix
def getFlowMatrix(numFeats1, numFeats2, w1, w2, D):
    """
    ...
    cons1 = [{'type': 'ineq', 'fun': positivity},
             {'type': 'eq', 'fun': maximiseTotalFlow, 'args': (w1, w2,)}]

    cons2 = [{'type': 'ineq', 'fun': fromSrc, 'args': (w1, i, shape,)} for i in range(numFeats1)]
    cons3 = [{'type': 'ineq', 'fun': toTgt, 'args': (w2, j, shape,)} for j in range(numFeats2)]
...
# Runs EMD program
def getEMD(dis_mat, numFeats1, numFeats2, w1, w2):
    """
    ...
    """
    print(dis_mat.shape)
    print(numFeats1, numFeats2)
    print(w1.shape)
    print(w2.shape)

    D = dis_mat
    F = getFlowMatrix(numFeats1, numFeats2, w1, w2, D)
