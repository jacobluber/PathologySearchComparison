...
def seg_and_patch(source, save_dir, patch_save_dir, mask_save_dir, stitch_save_dir,
    ...
    for i in range(total):
        ...
        # We set PATCH_SIZE and STEP_SIZE to 1024 for 20x slide and to 2048 for 40x slide.
        try:
            objective_power = int(WSI_object.wsi.properties['openslide.objective-power'])
        except KeyError:
            objective_power = 20
        ...
        if w * h > 5e8 and slide not in ['slide2.svs']:
            print('level_dim {} x {} is likely too large for successful segmentation, aborting'.format(w, h))
            df.loc[idx, 'status'] = 'failed_seg'
            continue
            
        elif w * h > 5e8 and slide in ['slide2.svs']: # Just to address TEST_DATA/UCLA/slide2.svs
            ...
            for i in range(4):
                ...
                tif_save_dir = dirname(patch_save_dir)
                if not isfile(join(tif_save_dir, f'{slide_id}_{i}.tif')):
                    ...
                    pv.tiffsave(join(tif_save_dir, f'{slide_id}_{i}.tif'), tile=True, pyramid=True, compression='none')
                new_WSI_object = WholeSlideImage(join(tif_save_dir, f'{slide_id}_{i}.tif'), hdf5_file=None)
