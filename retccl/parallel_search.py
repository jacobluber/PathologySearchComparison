...
def process_fname(fname, mosaics, test_mosaics, metadata, site, weight, cosine_threshold, temp_results_dir):
...
def worker(args):
    fname, mosaics, test_mosaics, metadata, site, weight, cosine_threshold, temp_results_dir = args
    return fname, *process_fname(fname, mosaics, test_mosaics, metadata, site, weight, cosine_threshold, temp_results_dir)
...
def wsi_query(mosaics, test_mosaics, metadata, site, weight, cosine_threshold, results_dir, temp_results_dir):
    ...
    args_list = [(fname, mosaics, test_mosaics, metadata, site, weight, cosine_threshold, temp_results_dir) for fname in test_mosaics.index.unique()]
...
if __name__ == "__main__":
    ...
    TEST_SLIDES_DIR = join("/raid/nejm_ai/TEST_DATA", experiment)
    ...
    RESULTS_DIR = join(BASE, "TEST_DATA", experiment, "results")
    ...
    for query, site in query_slides.items():
        ...
        site_test_mosaics = test_mosaics.loc[query].copy()
        Results, Bags, Entropies, Etas = wsi_query(mosaics, site_test_mosaics, metadata, site, weight, cosine_threshold, results_dir, temp_results_dir)
